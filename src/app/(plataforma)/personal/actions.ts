...
const noAutorizado: PersonalEstadoAccion = {
  ok: false,
  mensaje: "No tenés permisos para modificar este personal.",
};
...
export async function crearPersonal(
  ...
  if (!areaId || nombre.length < 2) {
    return { ok: false, mensaje: "Completá como mínimo nombre y dependencia." };
  }
...
export async function editarPersonal(
  ...
  if (!id || !areaId || nombre.length < 2) {
    return { ok: false, mensaje: "Revisá nombre y dependencia." };
  }

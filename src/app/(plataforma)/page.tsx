...
import { accesos, areas } from "@/lib/cultura-data";
...
export default async function InicioPage() {
  const proximas = await obtenerProximasEntradas(4);
  return <div className="page-stack">
    ...
    <section className="notice"><CircleAlert size={20}/><div><strong>Preparación para octubre de 2026</strong><span>La información documental está cargada como antecedente y debe ser validada por cada área.</span></div></section>
    <section><div className="section-heading"><div><span className="eyebrow">Estructura institucional</span><h2>Áreas culturales</h2></div><Link href="/areas">Ver todas <ArrowRight size={16}/></Link></div><div className="area-grid">{areas.map(({slug,nombre,descripcion,icono:Icon,color})=><Link href={`/areas/${slug}`} className={`area-card accent-${color}`} key={slug}><span className="area-icon"><Icon size={26}/></span><ArrowRight className="area-arrow" size={18}/><h3>{nombre}</h3><p>{descripcion}</p></Link>)}</div></section>

...
export default async function SolicitudesPage() {
  ...
  return <div className="page-stack">
    <section className="page-heading"><div><span className="eyebrow">Seguimiento interno</span><h1>Solicitudes</h1><p>Necesidades de cada dependencia con seguimiento de Coordinación y Subsecretaría.</p></div><Link className="button primary" href="/registrar"><Plus size={18}/>Nueva solicitud</Link></section>
    ...
    {completadas.length > 0 ? <section className="panel request-panel completed"><div className="section-heading compact"><div><span className="eyebrow">Historial reciente</span><h2>Completadas</h2></div></div><div className="request-list">{completadas.map((item) => <article className="request-row" key={item.id}>
      ...
      <div className="request-body"><div className="request-title"><strong>{item.title}</strong></div><p>{item.details}</p><small>{nombresAreas.get(item.area_id) ?? "Dependencia sin identificar"} · {mostrarFecha(item.due_date)}</small><div className="request-actions"><Link className="request-action" href={`/solicitudes/${item.id}/editar`}><Pencil size={16}/><span>Editar</span></Link><DeleteRequestButton id={item.id} titulo={item.title}/></div></div>

    ...
    <div className="page-stack">
      <section className="page-heading">
        <div>
          ...
          <p>Estructura institucional editable y conectada con agenda, solicitudes, registros y carga de información.</p>
        </div>
      </section>

      {editable ? <AreaCreateForm /> : (
        <section className="notice neutral-notice">
          <ShieldCheck size={20} />
          <div><strong>Vista de consulta</strong><span>La edición de la estructura está reservada a Coordinación.</span></div>
        </section>
      )}

      <section>
        <div className="section-heading">
          <div><span className="eyebrow">Estructura vigente</span><h2>Dependencias culturales</h2></div>
          <span className="structure-total">{areas.length} {areas.length === 1 ? "dependencia" : "dependencias"}</span>
        </div>
        <div className="area-grid large">
          {areas.map(({ slug, nombre, descripcion, icono: Icon, color, espacios }) => (
            <Link href={`/areas/${slug}`} className={`area-card accent-${color}`} key={slug}>
              ...
              <h2>{nombre}</h2>
              <p>{descripcion}</p>
              <small>{espacios.length} {espacios.length === 1 ? "espacio identificado" : "espacios identificados"}</small>
            </Link>
          ))}
        </div>
      </section>

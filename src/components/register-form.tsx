...
export function RegisterForm({ areas, tipoInicial = "" }: { areas: AreaFormulario[]; tipoInicial?: string }) {
  ...
  return <form className="register-form panel" action={formAction}>
    ...
    <div className="form-grid">
      ...
      <label><span>Área responsable</span><select name="area_id" required defaultValue="" onChange={(event) => setAreaSeleccionada(event.target.value)}><option value="" disabled>Seleccionar área</option>{areas.map(area => <option value={area.id} key={area.id}>{area.nombre}</option>)}</select></label>
      <label><span>Espacio específico</span><select name="space_id" defaultValue="" disabled={!areaSeleccionada}><option value="">Todo el área / no corresponde</option>{espacios.map(espacio => <option value={espacio.id} key={espacio.id}>{espacio.nombre}</option>)}</select></label>
    ...
    <div className="form-actions">
      {estado.mensaje ? <span className={estado.ok ? "form-success" : "form-error"}>{estado.ok ? <CheckCircle2 size={18}/> : <CircleAlert size={18}/>} {estado.mensaje}</span> : <span>Tipo, área, título y detalle son obligatorios.</span>}
